...
function printFaktur(data, user) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Faktur Pelunasan Hutang - ${esc(data?.kodepelunasan)}</title>
...
<div class="head"><div><h1>${esc(user?.namatenant || 'GRFYN POS')}</h1><div class="muted">${esc(user?.tenant_alamat || user?.alamat || '')}</div></div><div><h2>FAKTUR PELUNASAN HUTANG</h2><div class="muted">${esc(data?.kodepelunasan)}</div></div></div>
<div class="info"><div class="label">Supplier</div><div>${esc(data?.namasupplier || '-')}</div><div class="label">Tanggal</div><div>${esc(String(data?.tgltrans || '').slice(0, 10))}</div><div class="label">Metode</div><div>${esc(data?.metodbayar || '-')}</div><div class="label">Status</div><div>${esc(data?.status || '-')}</div><div class="label">Catatan</div><div>${esc(data?.catatan || '-')}</div></div>
...
export default function PelunasanHutang() {
  ...
  useEffect(() => { api.get('/supplier').then(r => setSuppliers(r.data)).catch(() => {}); }, []);

  const { page, setPage, totalPages, paginatedItems, resetPage } = usePagination(data, 20);
  useEffect(() => { resetPage(); }, [selectedSupplier, tglAwal, tglAkhir]);

  const openForm = (editData = null) => {
    openOrFocusTab({
      label: editData ? `Edit ${editData.kodepelunasan}` : 'Pelunasan Hutang Baru',
      icon: Plus,
      component: PelunasanHutangForm,
      props: { onSuccess: loadData, editData },
      type: editData ? 'form_edit' : 'form_add',
      kodemenu: editData ? `pelunasanhutang-edit-${editData.idpelunasan}` : 'pelunasanhutang-add',
    });
  };
  ...
  const handleEdit = async (row) => {
    ...
    try {
      const { data } = await api.get(`/pelunasanhutang/${row.idpelunasan}`);
      openForm(data);
    } catch {
      toast.error('Gagal memuat pelunasan hutang');
    }
  };
  ...
  const action = async (e, row, type) => {
    ...
    const map = {
      ...
      batal: { url: 'batal', title: 'Batalkan Pelunasan Hutang', msg: 'Batalkan pelunasan hutang DRAFT ini?', ok: 'Batalkan', access: 'bataltransaksi' },
    }[type];
  ...
  const handleCetak = async () => {
    ...
    const { data } = await api.get(`/pelunasanhutang/${selectedId}`);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 pt-4 pb-2 shrink-0">
        <div><h2 className="text-xl font-bold text-dark-500">Pelunasan Hutang</h2><p className="text-sm text-dark-300">Kelola pelunasan hutang ke supplier</p></div>
      ...
      <div className="px-6 pb-3 shrink-0">
        <div className="bg-white rounded-2xl border border-primary-50 p-3 grid grid-cols-2 gap-3 md:grid-cols-3">
          <div><label className="block text-[10px] font-semibold text-dark-300 mb-1">Supplier</label><select value={selectedSupplier} onChange={e => setSelectedSupplier(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-primary-100 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500/20"><option value="">Semua Supplier</option>{suppliers.map(s => <option key={s.idsupplier} value={s.idsupplier}>{s.namasupplier}</option>)}</select></div>
