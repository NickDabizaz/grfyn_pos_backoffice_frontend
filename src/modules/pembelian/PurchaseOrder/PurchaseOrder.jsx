...
export default function PurchaseOrder() {
  ...
  const handleApprove = async () => {
    ...
    if (!confirmed) return;
    try {
      await api.put(`/purchase-order/${selectedRow.idpo}/approve`);
  ...
  const handleBatalTransaksi = async () => {
    ...
    if (!confirmed) return;
    try {
      await api.put(`/purchase-order/${selectedRow.idpo}/batal`);
  ...
  const handleUnapprove = async () => {
    ...
    if (!confirmed) return;
    try {
      await api.put(`/purchase-order/${selectedRow.idpo}/unapprove`);
            ...
            <table className="w-full">
              ...
              <tbody>
                {paginatedItems.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-12 text-center text-sm text-dark-300">Tidak ada data purchase order</td></tr>
                )}
