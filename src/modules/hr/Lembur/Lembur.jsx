...
export default function Lembur() {
  ...
  const handleEdit = (d) => {
    if (!d) return;
    if (d.status !== 'DRAFT') { toast.error('Hanya lembur DRAFT yang bisa diedit'); return; }
    openTab({ label: `Edit Lembur ${d.namakaryawan}`, icon: Pencil, component: LemburForm, props: { mode: 'edit', id: d.idlembur, data: d, onSuccess: load }, type: 'form_edit' });
  };

  const handleRowClick = (d) => {
    const now = Date.now();
    const last = lastRowClickRef.current;
    if (last.id === d.idlembur && now - last.at < 400) {
      lastRowClickRef.current = { id: null, at: 0 };
      handleEdit(d);
      return;
    }
    lastRowClickRef.current = { id: d.idlembur, at: now };
    setSelectedId(d.idlembur === selectedId ? null : d.idlembur);
  };

  const handleApprove = async () => {
    if (!selectedRow) return;
    const ok = await confirm({ message: `Approve lembur ${selectedRow.namakaryawan} tgl ${selectedRow.tgllembur}?` });
    if (!ok) return;
    try {
      await api.put(`/lembur/${selectedRow.idlembur}/approve`);
  ...
  const handleDelete = async () => {
    ...
    if (!ok) return;
    try {
      await api.delete(`/lembur/${selectedRow.idlembur}`);
  ...
  const selectedRow = data.find((d) => d.idlembur === selectedId);
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedItems.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-10 text-center text-sm text-dark-300">Tidak ada data lembur</td></tr>
                  )}
                  {paginatedItems.map((d) => {
                    const isSelected = selectedId === d.idlembur;
                    return (
                      <tr key={d.idlembur}
