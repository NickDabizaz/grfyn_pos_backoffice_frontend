...
export default function Karyawan() {
  ...
  const handleTambah = () => openOrFocusTab({ label: 'Karyawan Baru', icon: Plus, component: KaryawanForm, props: { onSuccess: loadData }, type: 'form_add', kodemenu: 'karyawan-add' });
  const handleEdit = (k) => openOrFocusTab({ label: `Edit ${k.namakaryawan}`, icon: Pencil, component: KaryawanForm, props: { existingData: k, onSuccess: loadData }, type: 'form_edit', kodemenu: `karyawan-edit-${k.idkaryawan}` });
  ...
  const handleHapus = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Gagal hapus karyawan');
    }
  };
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 pt-4 pb-2 shrink-0">
        <div>
          ...
          <p className="text-sm text-dark-300">Master karyawan per lokasi</p>
        </div>
        <div className="flex items-center gap-2">
          {canTambah && <button onClick={handleTambah} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-accent-500 hover:bg-accent-600 text-white text-sm font-semibold"><Plus className="w-4 h-4" /> Karyawan Baru</button>}
          {selectedRow && canHapus && <button onClick={handleHapus} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm font-semibold hover:bg-red-100"><Trash2 className="w-4 h-4" /> Hapus Karyawan</button>}
      ...
      <div className="px-6 pb-3 shrink-0">
        <div className="bg-white rounded-2xl border border-primary-50 p-3">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari kode, nama, email, HP..." className="w-full pl-8 pr-3 py-2 rounded-lg border border-primary-100 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500/20" />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto px-6 pb-4">
        <div className="bg-white rounded-2xl border border-primary-50 overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {paginatedItems.length === 0 && <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-dark-300">Tidak ada data karyawan</td></tr>}
              {paginatedItems.map(k => (
                <tr key={k.idkaryawan} onClick={() => handleRowClick(k)} onDoubleClick={() => canUbah && handleEdit(k)} className={`border-b border-primary-50/50 text-sm cursor-pointer select-none transition-colors ${selectedId === k.idkaryawan ? 'bg-primary-50 ring-1 ring-inset ring-primary-200' : 'hover:bg-warm-50/30'}`}>
                  ...
                  <td className="px-4 py-3 text-dark-400 text-xs">{k.namalokasi || '-'}</td>
                  <td className="px-4 py-3 text-dark-400 text-xs">{k.email || '-'}</td>
                  <td className="px-4 py-3 text-dark-400 text-xs">{k.hp || '-'}</td>
                  <td className="px-4 py-3 text-right text-dark-500 text-xs">{formatRupiah(k.gaji)}</td>
                  <td className="px-4 py-3 text-center"><span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${k.status === 'AKTIF' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500'}`}>{k.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
