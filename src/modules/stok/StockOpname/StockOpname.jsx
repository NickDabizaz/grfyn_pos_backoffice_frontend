...
import { Plus, Search, RefreshCw, CheckCircle, Printer, XCircle } from 'lucide-react';
...
function printFakturOpname(data, user) {
  const rows = data?.items || [];
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Faktur Opname Stok - ${esc(data?.kodestockopname)}</title>
...
<div class="head"><div><h1>${esc(user?.namatenant || 'GRFYN POS')}</h1><div class="muted">${esc(user?.tenant_alamat || user?.alamat || '')}</div><div class="muted">${esc(user?.tenant_hp || user?.hp || '')}</div></div><div><h2>FAKTUR OPNAME STOK</h2><div class="muted">${esc(data?.kodestockopname)}</div></div></div>
<div class="info"><div class="label">Kode</div><div>${esc(data?.kodestockopname)}</div><div class="label">Tanggal</div><div>${esc(String(data?.tgltrans || '').slice(0, 10))}</div><div class="label">Lokasi</div><div>${esc(data?.namalokasi || '-')}</div><div class="label">Status</div><div>${esc(data?.status || '-')}</div><div class="label">Catatan</div><div>${esc(data?.catatan || '-')}</div></div>
<table><thead><tr><th class="c" style="width:34px">No</th><th style="width:92px">Kode</th><th>Nama Barang</th><th class="c" style="width:60px">Satuan</th><th class="r" style="width:78px">Sistem</th><th class="r" style="width:78px">Fisik</th><th class="r" style="width:78px">Selisih</th></tr></thead><tbody>
${rows.map((item, i) => `<tr><td class="c">${i + 1}</td><td>${esc(item.kodebarang || '')}</td><td>${esc(item.namabarang || '')}</td><td class="c">${esc(item.satuan || item.satuankecil || '')}</td><td class="r">${num(item.stok_sistem)}</td><td class="r">${num(item.stok_fisik)}</td><td class="r">${num(item.selisih)}</td></tr>`).join('')}
</tbody></table><div class="sign"><div><div>Dibuat Oleh</div><div class="line">${esc(user?.namauser || user?.username || '')}</div></div><div><div>Disetujui Oleh</div><div class="line">&nbsp;</div></div></div></body></html>`;
...
export default function StockOpname() {
  ...
  const handleEdit = async (row) => {
    ...
    try {
      const { data } = await api.get(`/stock-opname/${row.idstockopname}`);
      openOrFocusTab({
        label: `Edit ${data.kodestockopname}`,
        icon: Plus,
        component: StockOpnameForm,
        props: { onSuccess: loadData, editData: data },
        type: 'form_edit',
        kodemenu: `stockopname-edit-${data.idstockopname}`,
      });
    } catch {
      toast.error('Gagal memuat stock opname');
    }
  };
  ...
  const handleFinalize = async (e, id) => {
    ...
    const ok = await confirm({ title: 'Approve Stock Opname', message: 'Approve stock opname ini? Selisih stok akan masuk ke kartu stok.', confirmText: 'Approve', cancelText: 'Batal', variant: 'primary' });
    if (!ok) return;
    try {
      await api.put(`/stock-opname/${id}/finalize`);
      toast.success('Opname stok diapprove & penyesuaian stok dibuat');
      loadData();
    } catch (err) {
  ...
  const handleUnapprove = async (e, id) => {
    ...
    const ok = await confirm({ title: 'Batal Approve Opname', message: 'Kembalikan stock opname ini ke DRAFT? Mutasi kartu stok opname akan dihapus.', confirmText: 'Batal Approve', cancelText: 'Tutup', variant: 'danger' });
    if (!ok) return;
    try {
      await api.put(`/stock-opname/${id}/unapprove`);
      toast.success('Approve stock opname dibatalkan');
      loadData();
    } catch (err) {
  ...
  const handleCetak = async () => {
    ...
    try {
      const { data } = await api.get(`/stock-opname/${selectedId}`);
      printFakturOpname(data, user);
    } catch {
      toast.error('Gagal memuat faktur opname stok');
    }
  };

  const selectedRow = list.find(so => so.idstockopname === selectedId);
            ...
            <table className="w-full">
              ...
              <tbody>
                {paginatedItems.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-dark-300">Tidak ada data stock opname</td></tr>
                )}
                {paginatedItems.map((so) => (
                  <tr key={so.idstockopname}
                    onClick={() => setSelectedId(so.idstockopname)}
                    onDoubleClick={() => handleEdit(so)}
                    className={`border-b border-primary-50/50 text-sm cursor-pointer select-none transition-colors ${selectedId === so.idstockopname ? 'bg-warm-100 ring-1 ring-inset ring-warm-200' : 'hover:bg-warm-50/30'}`}>
                    ...
                    <td className="px-4 py-3 text-dark-500">{so.namalokasi || '-'}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      ...
                      {so.status === 'APPROVED' && canAccess(access, 'batalapprove') && (
                        <button onClick={(e) => handleUnapprove(e, so.idstockopname)} className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold bg-amber-50 text-amber-600 hover:bg-amber-100 mx-auto">
                          <XCircle className="w-3 h-3" /> Batal Approve
                        </button>
                      )}
                      {so.status === 'DRAFT' && canAccess(access, 'approve') && (
