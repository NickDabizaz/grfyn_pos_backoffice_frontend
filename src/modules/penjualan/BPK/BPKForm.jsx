...
export default function BPKForm({ onSuccess, tabId, editData }) {
  ...
  const [idso, setIdso]         = useState(editData?.idso || null);
  const [kodeso, setKodeso]     = useState(editData?.kodeso || '');
  ...
  const handleSubmit = async (approve = false) => {
    if (isLocked) return toast.error('BPK yang sudah approve tidak bisa disimpan lagi');
    if (!idso) return toast.error('Kode SO (Referensi) wajib dipilih');
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/bpk-jual/${editData.idbpk}`, payload);
        toast.success(approve ? 'BPK berhasil disimpan dan diapprove!' : 'BPK berhasil diupdate!');
      } else {
        await api.post('/bpk-jual', payload);
        toast.success(approve ? 'BPK berhasil disimpan dan diapprove!' : 'BPK berhasil dibuat!');
      }
      ...
      <div className="flex items-center gap-3 px-6 pt-4 pb-3 border-b border-primary-50 shrink-0">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-bold text-dark-500">{isEdit ? `Edit ${editData?.kodebpk || 'BPK'}` : 'BPK Baru'}</h2>
          ...
          <p className="text-xs text-dark-300">{isEdit ? 'Edit pengeluaran barang' : 'Pengeluaran barang berdasarkan SO'}</p>
        </div>
      </div>
            ...
            <div className="p-5 grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 order-3">
                <label className="block text-xs font-semibold text-dark-400 mb-1.5">
                  Kode SO (Referensi) <span className="text-red-500">*</span>
                </label>
                <div className="flex items-center gap-2">
                  <div className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl border text-sm min-h-[38px] ${idso ? 'border-primary-100 bg-warm-50/40' : 'border-dashed border-amber-300 bg-amber-50/40'}`}>
                    <FileText className="w-3.5 h-3.5 text-dark-300 shrink-0" />
                    {kodeso
                      ? <span className="font-mono text-dark-500 text-xs">{kodeso}</span>
                      : <span className="text-dark-300 text-xs">Pilih SO terlebih dahulu...</span>
                    }
                  </div>
                  ...
                  {kodeso && (
                    <button onClick={() => { setIdso(null); setKodeso(''); }}
                  ...
                  {computedItems.length === 0 ? (
                    <tr>
                      <td colSpan={11} className="px-4 py-12 text-center text-sm text-dark-300">
                        Pilih SO di atas untuk otomatis mengisi, atau klik <span className="font-semibold text-primary-500">Tambah Barang</span>.
                      </td>
                    </tr>
