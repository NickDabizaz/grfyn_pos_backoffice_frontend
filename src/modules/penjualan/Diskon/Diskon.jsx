...
import { Plus, Trash2, Search, RefreshCw, Tag } from 'lucide-react';
...
export default function Diskon({ kodemenu }) {
  ...
  const menuCode = kodemenu === 'penjualan.diskon' ? 'master.promo' : (kodemenu || 'master.promo');
  const { access } = useMenuAccess(menuCode);
  ...
  useEffect(() => {
    if (selectedId && !filtered.some((d) => d.idiskon === selectedId)) setSelectedId(null);
  }, [filtered, selectedId]);

  const handleRefresh = async () => { setRefreshing(true); await load(); setRefreshing(false); };

  const handleTambah = () => {
    openTab({ label: 'Tambah Promo', icon: Plus, component: DiskonForm, props: { mode: 'add', onSuccess: load }, type: 'form_add' });
  };

  const handleEdit = (d) => {
    openTab({ label: `Edit ${d.kodediskon}`, icon: Tag, component: DiskonForm, props: { mode: 'edit', id: d.idiskon, data: d, onSuccess: load }, type: 'form_edit' });
  };

  const handleDelete = async (id) => {
    const ok = await confirm({ message: 'Hapus promo ini?' });
    if (!ok) return;
    try {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 pt-4 pb-2 shrink-0">
        <div>
          ...
          <p className="text-sm text-dark-300">Data promo yang digunakan pada transaksi penjualan</p>
        </div>
            ...
            <table className="w-full">
              <thead className="sticky top-0 z-10">
                <tr className="border-b border-primary-50 bg-warm-50/50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-dark-300">Nama Promo</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-dark-300">Tipe</th>
              ...
              <tbody>
                {paginatedItems.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-dark-300">Tidak ada data promo</td></tr>
                )}
                {paginatedItems.map((d) => (
                  <tr
                    key={d.idiskon}
                    onClick={() => setSelectedId(prev => prev === d.idiskon ? null : d.idiskon)}
                    ...
                    className={`border-b border-primary-50/50 text-sm cursor-pointer select-none transition-colors ${
                      selectedId === d.idiskon ? 'bg-primary-50 ring-1 ring-inset ring-primary-200' : 'hover:bg-warm-50/30'
                    }`}
                  >
